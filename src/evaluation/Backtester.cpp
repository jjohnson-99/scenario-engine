...
ModelEvaluationResult Backtester::evaluate(const Forecaster& model, const TimeSeries& series) const
{
    return evaluate(model, series, model.horizon());
}

ModelEvaluationResult
Backtester::evaluate(const Forecaster& model, const TimeSeries& series, std::size_t horizon) const
{
    ...
    return {
        ...
        .mape = ape_count > 0 ? (ape_sum / static_cast<double>(ape_count)) * 100.0
