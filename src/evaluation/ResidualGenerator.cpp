...
ResidualSeries ResidualGenerator::generate(const Forecaster& model, const TimeSeries& series) const
{
    return generate(model, series, model.horizon());
}

ResidualSeries
ResidualGenerator::generate(const Forecaster& model, const TimeSeries& series, std::size_t horizon) const
{
